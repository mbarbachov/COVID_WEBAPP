...
var parsePlaceId = function(value) {
    ...
    // - Short place IDs always start with "ChIJ" or "GhIJ" and have exactly 27 characters.
    // - Long place ID always start with "E" or "I" and have at more than 30 characters.
    if (!value.match(/^[a-zA-Z0-9_-]+$/)) {
        return null;
    }
    if ((value.length > 30) && (value[0] == 'E' || value[0] == 'I')) {
        return value;
    }
    if ((value.length = 27) &&
        (value.substring(0, 4) == 'ChIJ' || value.substring(0, 4) == 'GhIJ')) {
        return value;
    }
    return null;
};
